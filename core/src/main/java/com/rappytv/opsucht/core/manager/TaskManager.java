...
public class TaskManager {
  ...
  public Task compareSkullDateTask() {
    return TASK_CACHE.computeIfAbsent("compare-skull-date", (key) ->
        Task.builder(() -> {
          if (!this.addon.configuration().enabled().get() || !this.addon.server().isConnected()) {
            return;
          }
          ReminderConfig config = this.addon.configuration().reminderConfig();
          if (!config.skullReminders().get() || config.lastSkullClaim().get() == -1L) {
            return;
          }
          OPSuchtRank rank = this.addon.configuration().rank().get();
